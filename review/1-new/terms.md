...
- **np.percentile(arr, n)**: 배열의 n% 지점 값을 구하는 함수. IQR 이상치 탐지에 사용 (Q1=25%, Q3=75%).
- **IQR 이상치 탐지**: `upper = Q3 + 1.5 * (Q3-Q1)`. 약 99.3%를 정상 범위로 커버. 이 밖이면 이상치.
- **np.corrcoef(a, b)**: 두 배열의 상관계수 행렬 반환. `[0,1]` 위치가 실제 상관계수. +1=양의 상관, -1=음의 상관.
- **default_rng(seed)**: `np.random.default_rng(seed=42)` — 재현 가능한 난수 생성기. seed 고정 → 누가 돌려도 같은 결과.
- **rng.normal(loc, scale, size)**: 정규분포 난수 생성. loc=평균, scale=표준편차. 가짜 센서 데이터 만들기에 유용.
- **np.concatenate([a, b])**: 배열들을 합치는 함수. 반드시 리스트/튜플로 묶어서 전달.
- **np.linalg.solve(A, b)**: Ax=b 연립방정식을 푸는 함수. 센서 보정 계수 계산 등에 활용.
